...
type FromSkeleton<T extends ContentfulSkeletons> = Entry<T, 'WITHOUT_UNRESOLVABLE_LINKS'>['fields'];
...
type ContentfulResponse<ContentType extends ContentfulSkeletons["contentTypeId"]> =
  Entry<Extract<ContentfulSkeletons, { contentTypeId: ContentType }>, 'WITHOUT_UNRESOLVABLE_LINKS'>[];
 ...
 */
async function contentfulBaseQuery<ContentType extends ContentfulSkeletons["contentTypeId"]>(content_type: ContentType):
  Promise<{ data: ContentfulResponse<ContentType> } | { error: unknown }> {
  try {
    const res = await client.withoutUnresolvableLinks.getEntries<ContentfulSkeletons>({ content_type });
...
function createQuery<ContentType extends ContentfulSkeletons['contentTypeId']>(content_type: ContentType) {
  return {
    query: () => content_type,
    transformResponse: (response: ContentfulResponse<ContentType>) => response.map(entry => entry.fields)
  };
}
...
const contentful = createApi({
  ...
  endpoints: build => ({
    ...
    boardMessages: build.query<BoardMessage[], void>(createQuery('board-message')),
